...
def web_services_smoke_test(e_child_name, device_name, platform, package_id = '')
  ...
  context '9. Device log/game upload' do
    ...
    it "Verify 'Device Log Upload' calls successfully - #{LFSOAP::CONST_DEVICE_LOG_UPLOAD_LINK}" do
      expect(soap_fault1).to eq(0)
    end

    it "Verify 'Device Game Upload' calls successfully - #{LFSOAP::CONST_GAME_LOG_UPLOAD_LINK}" do
