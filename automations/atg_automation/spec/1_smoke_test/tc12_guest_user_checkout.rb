...
=begin
Verify that user can check out as guest account
=end
...
feature "TC12 - Guest user checkout - ENV = '#{Data::ENV_CONST}' - Locale = '#{Data::LOCALE_CONST}'", js: true do
  ...
  context 'Check out as guest' do
    scenario '1. Pre-condition: Add item to cart' do
      prod_info = atg_home_page.get_random_product_info
      atg_home_page.add_to_cart prod_info[:id]
      atg_checkout_page = atg_home_page.goto_checkout
    end

    scenario '2. Go to Shipping page' do
      atg_shipping_page = atg_checkout_page.checkout_asguest(email_address)
    end

    scenario '3. Go to Payment page' do
    ...
    scenario '4. Go to Review page' do
    ...
    scenario '5. Go to Confirmation page' do
      ...
      atg_confirmation_page.record_order_id email_address
    end
  end
  ...
  context 'Verify information on Confirmation page' do
    ...
    scenario '4. Verify order summary info' do
      expect(overview_info[:summary]).to eq(ProductInformation::ORDER_SUMMARY_TEXT_CONST % [email_address, shipping_method[:text_on_confirmation]])
    end
  end

  context 'Verify information on Email page' do
    before :all do
      mail_detail_page = mail_home_page.go_to_mail_detail email_address
    end
  ...
  context 'Verify information on CSC tool' do
    ...
    scenario '3. Verify email address' do
      expect(csc_home_page.order_view.email_address_txt.text).to eq(email_address)
    end
