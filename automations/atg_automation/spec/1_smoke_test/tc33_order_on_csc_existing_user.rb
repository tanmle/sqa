...
=begin
Verify user can create a order on CSC page with an existing account
=end
...
feature "TC33 - Order on CSC - Ensure that an existing user able to place order on CSC - ENV = '#{Data::ENV_CONST}' - Locale = '#{Data::LOCALE_CONST}'", js: true do
  ...
  context 'Order on CSC page', js: true do
    scenario '1. Select customer', js: true do
      expect(csc_home_page.select_customer_by_email(Data::EMAIL_EXIST_FULL_CONST)).to eq(true)
    end
    ...
    scenario '3. Add to cart 1 item', js: true do
      added_to_cart_info = csc_home_checkout_page.add_to_cart(prod_info[:id], qty) # prod_info[:id]
    end

    scenario '4. Check out', js: true do
      check_out_info = csc_home_checkout_page.check_out(Data::EMAIL_EXIST_FULL_CONST, \
  ...
  context 'Verify information on CSC tool', js: true do
    ...
    scenario '3. Verify email address', js: true do
      expect(order_view_info[:email]).to eq(Data::EMAIL_EXIST_FULL_CONST)
    end
  ...
  context 'Verify information on Email page', js: true do
    before :all, js: true do
      email_part1 = Data::EMAIL_EXIST_FULL_CONST[0..Data::EMAIL_EXIST_FULL_CONST.index('@') - 1]
    ...
    scenario '1. Verify subject of email' do
      expect(email_order_info[:subject]).to eq('New Order Confirmation Email')
    end

    scenario '2.Verify order number', js: true do
      expect(ema il_order_info[:id]).to include(check_out_info[:id])
    end
    ...
    context 'Verify shipping details' do
      ...
      scenario '2. SKU' do
        expect(email_order_info[:shipping_detail]).to include(added_to_cart_info[:sku])
      end

      scenario '3. Shipping method' do
        expect(email_order_info[:shipping_detail]).to include((ProductInformation::SHIPPING_METHOD_CONST % '').strip)
      end

      scenario '4. Shipping address' do
